...
import { auth, db, storage } from "@/config/firebaseConfig"; // Configurações do Firebase
...
// ===============================================
// Funções para Configurações (continuam LOCAIS)
// ===============================================
...
// ===============================================
// Funções de Armazenamento LOCAL (AsyncStorage) - Padrão do App
// ===============================================
...
const saveTitles = async (titles: Title[]) => { // Tornada interna, não exportada
    try {
...
export const addTitle = async (newTitleData: Omit<Title, 'id' | 'thumbnailUri' | 'coverUri'> & { coverUri: string | null }): Promise<void> => {
    ...
    const newTitle: Title = {
        ...
        coverUri: newTitleData.coverUri,     // Apenas guarda a URI local/externa
        thumbnailUri: newTitleData.coverUri, // Usa a mesma para thumbnail por enquanto
...
const uploadImageToCloud = async (uri: string, titleId: string): Promise<string> => {
    const user = auth.currentUser;
    if (!user) throw new Error('Usuário não autenticado para fazer upload da imagem.');

    // Se a URI já for uma URL do Storage, não precisamos fazer upload novamente
    ...
    const fileRef = ref(storage, `users/${user.uid}/covers/${titleId}.jpg`);
    await uploadBytes(fileRef, blob);
    blob.close(); // Fecha o blob

    return await getDownloadURL(fileRef);
};

const deleteImageFromCloud = async (id: string, coverUri: string) => {
    const user = auth.currentUser;
    if (!user) throw new Error("Usuário não autenticado para deletar imagem.");

    try {
        if (coverUri && coverUri.startsWith('http')) { // Apenas tenta deletar se for uma URL do Storage
            const imageRef = ref(storage, `users/${user.uid}/covers/${id}.jpg`);
            await deleteObject(imageRef);
        }
    } catch (e) {
        console.error("Erro ao deletar imagem do Storage (pode não existir): ", e);
    }
};
...
export const addTitleToCloud = async (newTitleData: Omit<Title, 'id' | 'thumbnailUri' | 'coverUri'> & { coverUri: string | null }): Promise<void> => {
    const newId = uuidv4();
    let coverUrl = newTitleData.coverUri || null;

    if (newTitleData.coverUri && !newTitleData.coverUri.startsWith('http')) { // Se for local, faz upload
        coverUrl = await uploadImageToCloud(newTitleData.coverUri, newId);
    }
...
export const updateTitleInCloud = async (updatedTitle: Title): Promise<void> => {
    ...
    // Se a imagem foi alterada para um arquivo local, faz novo upload
    if (updatedTitle.coverUri && !updatedTitle.coverUri.startsWith('http')) {
        coverUrl = await uploadImageToCloud(updatedTitle.coverUri, updatedTitle.id);
    }

    const titleToUpdate = { ...updatedTitle, coverUri: coverUrl, thumbnailUri: coverUrl };
...
export const deleteTitleFromCloud = async (id: string, coverUri: string | null) => {
    const user = auth.currentUser;
    if (!user) throw new Error("Usuário não autenticado para deletar da nuvem.");

    try {
        await deleteDoc(doc(getUserTitlesCollection(), id));
        if (coverUri) {
            await deleteImageFromCloud(id, coverUri);
        }
    } catch (e) {
...
export const syncLocalToCloud = async () => {
    ...
    for (const [id, title] of cloudTitleMap.entries()) {
        await deleteTitleFromCloud(id, title.coverUri || null);
    }
};
